    ...
    <>
      <div style={{ backgroundColor: C, width: '100%', padding: '7px 0' }}>
        ...
        <div className="md:hidden flex justify-center py-0.5">
           <p className="text-brand-cream text-[10px] font-medium tracking-wider uppercase">⭐ {PROMO} ⭐</p>
        </div>
      </div>
      ...
      <header style={{ backgroundColor: '#FDF3E7', borderBottom: '1px solid #ebebeb', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', position: 'sticky', top: 0, zIndex: 100 }}>
            ...
            {mainNavLinks.map(link => (
              ...
              >
                <Link 
                  ...
                  className={`flex items-center gap-1 px-2.5 text-[12px] font-bold tracking-widest transition-all uppercase ${location.pathname === link.href ? 'text-[#E84949]' : 'text-[#333] hover:text-[#E84949]'}`}
                >
                ...
                <AnimatePresence>
                  {activeMenu === link.name && link.mega && (
                    <motion.div 
                      ...
                      className="absolute top-full left-1/2 -translate-x-1/2 w-200 xl:w-250 bg-[#FDF4E6] shadow-2xl rounded-b-3xl border-t-2 border-[#E84949] p-8 grid grid-cols-4 gap-8 z-50 overflow-hidden"
                    >
                        ...
                        <div key={idx} className="col-span-1">
                              ...
                              {col.items.map(item => (
                                <li key={item}>
                                  <Link 
                                    ...
                                    onClick={handleLinkClick}
                                    className="text-[12px] text-[#555] p-2 -mx-2 rounded-lg hover:text-[#E84949] hover:bg-[#F9EAD3] hover:translate-x-1 transition-all block font-bold capitalize"
                                  >
                                    {item}
                                  </Link>
                                </li>
                          ...
                          {col.banner && (
                            <Link 
                              to={link.href} 
                              onClick={handleLinkClick}
                              className="rounded-2xl overflow-hidden aspect-4/5 relative group/banner cursor-pointer shadow-md block"
                            >
                              ...
                              <div className="absolute inset-0 bg-linear-to-t from-black/60 to-transparent p-4 flex flex-col justify-end">
                                <span className="text-white text-[9px] uppercase font-bold tracking-widest">New Collection</span>
                                <h5 className="text-white font-grandstander text-[13px] font-bold">Shop {link.name}</h5>
                              </div>
                            </Link>
                          )}
                        </div>
                ...
                <AnimatePresence>
                  {activeMenu === link.name && link.dropdown && (
                    <motion.div 
                      ...
                      exit={{ opacity: 0, y: 10 }}
                      transition={{ duration: 0.2 }}
                      className="absolute top-full left-0 w-56 bg-[#FDF4E6] shadow-xl rounded-b-xl border-t-2 border-[#E84949] py-2 z-50"
                    >
                      {link.dropdown.map(sub => (
                        <Link 
                          ...
                          to={sub.href} 
                          onClick={handleLinkClick}
                          className="block px-5 py-2.5 text-[12px] text-[#555] hover:text-[#E84949] hover:bg-[#F9EAD3] transition-all font-bold uppercase tracking-wider"
                        >
                          {sub.name}
                        </Link>
                      ))}
        ...
        <AnimatePresence>
          {searchOpen && (
            <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden bg-[#FDF4E6] border-t border-[#333]/10">
               <div className="max-w-350 mx-auto px-4 md:px-10 py-6">
                  <div className="relative">
                    <input type="text" placeholder="Search for toys..." className="w-full h-14 bg-[#F9EAD3] border-none rounded-full px-8 text-[16px] outline-none placeholder-[#666]" autoFocus />
                ...
                {mainNavLinks.map(link => (
                  <div key={link.name}>
                    <div className="flex items-center justify-between px-6 py-4 border-b border-[#333]/5">
                      <Link to={link.href} className={`text-[13px] font-bold tracking-widest uppercase transition-colors ${location.pathname === link.href ? 'text-[#E84949]' : 'text-[#333]'}`}>{link.name}</Link>
                           ...
                           <div key={cidx}>
                             ...
                             {col.items && (
                               <div className="flex flex-col gap-2">
                                 {col.items.map(i => <Link key={i} to={`/collections/${i.toLowerCase().replaceAll(' ', '-')}`} className="text-[12px] text-[#333] font-bold hover:text-[#E84949] transition-colors">{i}</Link>)}
                               </div>
                             )}
                           </div>
                        )) : link.dropdown.map(sub => (
                           <Link key={sub.name} to={sub.href} className="block text-[12px] text-[#333] font-bold hover:text-[#E84949] transition-colors">{sub.name}</Link>
                        ))}
                      </motion.div>
                    )}
                  </div>
      ...
      {isPastHero && (
        <motion.div initial={{ y: 80 }} animate={{ y: 0 }} className="lg:hidden fixed bottom-0 left-0 right-0 bg-[#FDF4E6] shadow-[0_-8px_24px_rgba(0,0,0,0.08)] z-150 px-4 py-2 border-t border-[#333]/10 flex items-center justify-between">
           <Link to="/" className="flex flex-col items-center gap-1 min-w-16">
             <Home size={18} className={location.pathname === '/' ? 'text-[#E84949]' : 'text-[#666]'} />
             <span className="text-[9px] font-bold uppercase tracking-widest">Home</span>
           </Link>
           <button onClick={() => setMobileOpen(true)} className="flex flex-col items-center gap-1 min-w-16">
             <Menu size={18} className="text-[#666]" />
             <span className="text-[9px] font-bold uppercase tracking-widest">Menu</span>
           </button>
           <div className="relative -top-6">
              <Link to="/search" className="w-12 h-12 bg-[#E84949] text-white rounded-full flex items-center justify-center shadow-xl border-4 border-[#FDF4E6]">
                 <Search size={20} />
              </Link>
           </div>
           ...
           <button className="flex flex-col items-center gap-1 min-w-16">
             <ShoppingCart size={18} className="text-[#666]" />
